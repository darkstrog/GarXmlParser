...
{
    public class RoomWriter: IEntityWriter
    {
        public string InsertQuery => """
                                        COPY room
                                            (id, objectid, objectguid, changeid, roomnumber, 
                                            roomtype, opertypeid, previd, nextid, updatedate,
                                            startdate, enddate, isactual, isactive)
                                        FROM STDIN (FORMAT BINARY)
                                        """;

        public string FilePattern => @"AS_ROOMS_\d{8}_[0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}\.XML$";
        ...
        public void WriteRow(NpgsqlBinaryImporter writer, XElement entity)
        {
            ...
            writer.Write((int)entity.Attribute(_ROOMTYPEName), NpgsqlDbType.Integer);
