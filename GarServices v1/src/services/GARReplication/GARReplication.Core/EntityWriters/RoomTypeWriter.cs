...
{
    public class RoomTypeWriter: IEntityWriter
    {
        public string InsertQuery => """
                                        COPY room_type
                                            (id, name, shortname, description, updatedate,
                                            startdate, enddate, isactive)
                                        FROM STDIN (FORMAT BINARY)
                                        """;

        public string FilePattern => @"AS_ROOMS_TYPES_\d{8}_[0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}\.XML$";
