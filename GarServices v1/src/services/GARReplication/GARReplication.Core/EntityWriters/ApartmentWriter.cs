...
{
    public class ApartmentWriter:IEntityWriter
    {
        ...
        public string FilePattern => @"AS_APARTMENTS_\d{8}_[0-9a-fA-F]{8}-([0-9a-fA-F]{4}-){3}[0-9a-fA-F]{12}\.XML$";
        ...
        public void WriteRow(NpgsqlBinaryImporter writer, XElement entity)
        {
            ...
            writer.Write((int)entity.Attribute(_APARTTYPEName), NpgsqlDbType.Integer);
            writer.Write((long)entity.Attribute(_OPERTYPEIDName), NpgsqlDbType.Bigint);
