...
{
    public class GarXmlProcessor: IGarXmlProcessor
    {
        private readonly IGarNodeParser _parser;
        private readonly ILogger? _logger;

        public GarXmlProcessor(IGarNodeParser garNodeParser ,ILogger<GarXmlProcessor> logger)
        {
            _parser = garNodeParser;
            _logger = logger;
        ...
        /// для мапинга используется переданный в метод IGarItemMapper<T> маппер.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        /////-------------------------------------------------------------/////
        /// <summary>
        /// Перегрузка StreamZipArchiveFilesAsync() не использующая маппер
        /// Возвращает XElement объекты асинхронно из файлов упакованных в zip архив
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="zipFilePath">Путь к zip архиву</param>
        /// <param name="regexPattern">Паттерн для поиска нужных XML в архиве</param>
        /// <param name="nodeName">Имя сущности в исходном xml</param>
        /// <param name="cancellationToken">Токен отмены операции</param>
        /// <param name="progress">Параметр для отслеживания прогресса выполнения парсинга</param>
        /// <returns></returns>
        public async IAsyncEnumerable<XElement> StreamZipArchiveFilesAsync(
            string zipFilePath,
            string regexPattern,
            string nodeName,
            [EnumeratorCancellation] CancellationToken cancellationToken = default,
            ...
            )
        {
            ...
            using (ZipArchive archive = new ZipArchive(zipStream, ZipArchiveMode.Read))
            {
                ...
                foreach (ZipArchiveEntry entry in matchingEntries)
                {
                    ...
                    using (Stream entryStream = entry.Open())
                    {
                        await foreach (var item in _parser.GetXmlObjectFromStreamAsync(entryStream, nodeName, _fileName, cancellationToken))
                        {
                            cancellationToken.ThrowIfCancellationRequested();
                            _totalItemsCount++;
                            if (_totalItemsCount % 100 == 0)
                            ...
                            ///возвращаем xelement для дальнейшей обработки
                            ///TODO: Решить нужен ли номер строки из парсера вообще
