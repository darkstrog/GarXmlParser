...
{
    public class MunHierarchyService: IMunHierarchyService
    {
        private readonly ILogger<MunHierarchyService> _logger;
        private readonly IMunHierarchyRepo _repository;
        private readonly IProblemDataService<MunHierarchyDto> _problemDataService;

        public MunHierarchyService(IMunHierarchyRepo repository, IProblemDataService<MunHierarchyDto> problemDataService, ILogger<MunHierarchyService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(MunHierarchyDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи MunHierarchy с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи MunHierarchy с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<MunHierarchyDto> entities, int batchSize = 1000)
        {
            var batch = new List<MunHierarchyDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<MunHierarchyDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<MunHierarchyDto> entities, int batchSize = 100)
        {
            var batch = new List<MunHierarchyDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<MunHierarchyDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(MunHierarchyDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(MunHierarchyDto record)
        {
