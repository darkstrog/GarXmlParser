...
namespace GAROperations.Infrastructure.Services
{
    public class ApartmentService:IApartmentService
    {
        ...
        private readonly IApartmentRepo _repository;
        private readonly IProblemDataService<ApartmentDto> _problemDataService;

        public ApartmentService(IApartmentRepo repository, IProblemDataService<ApartmentDto> problemDataService, ILogger<ApartmentService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(ApartmentDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи Apartment с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи Apartment  с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<ApartmentDto> entities, int batchSize = 1000)
        {
            var batch = new List<ApartmentDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<ApartmentDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<ApartmentDto> entities, int batchSize = 100)
        {
            var batch = new List<ApartmentDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<ApartmentDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(ApartmentDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(ApartmentDto record)
        {
