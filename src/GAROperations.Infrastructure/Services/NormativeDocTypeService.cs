...
{
    public class NormativeDocTypesService: INormativeDocTypesService
    {
        private readonly ILogger<NormativeDocTypesService> _logger;
        private readonly INormativeDocTypeRepo _repository;
        private readonly IProblemDataService<NormativeDocTypeDto> _problemDataService;

        public NormativeDocTypesService(INormativeDocTypeRepo repository, IProblemDataService<NormativeDocTypeDto> problemDataService, ILogger<NormativeDocTypesService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(NormativeDocTypeDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи NormativeDocType с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи NormativeDocType с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<NormativeDocTypeDto> entities, int batchSize = 1000)
        {
            var batch = new List<NormativeDocTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<NormativeDocTypeDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<NormativeDocTypeDto> entities, int batchSize = 100)
        {
            var batch = new List<NormativeDocTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<NormativeDocTypeDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(NormativeDocTypeDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(NormativeDocTypeDto record)
        {
