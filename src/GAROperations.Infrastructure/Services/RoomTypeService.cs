...
namespace GAROperations.Infrastructure.Services
{
    public class RoomTypeService: IRoomTypeService
    {
        ...
        private readonly IRoomTypeRepo _repository;
        private readonly IProblemDataService<RoomTypeDto> _problemDataService;

        public RoomTypeService(IRoomTypeRepo repository, IProblemDataService<RoomTypeDto> problemDataService, ILogger<RoomTypeService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(RoomTypeDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи RoomType с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи RoomType с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<RoomTypeDto> entities, int batchSize = 1000)
        {
            var batch = new List<RoomTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<RoomTypeDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<RoomTypeDto> entities, int batchSize = 100)
        {
            var batch = new List<RoomTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<RoomTypeDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(RoomTypeDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(RoomTypeDto record)
        {
