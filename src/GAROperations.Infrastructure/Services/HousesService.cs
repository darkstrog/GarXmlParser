...
namespace GAROperations.Infrastructure.Services
{
    public class HousesService: IHousesService
    {
        ...
        private readonly IHousesRepo _repository;
        private readonly IProblemDataService<HouseDto> _problemDataService;

        public HousesService(IHousesRepo repository, IProblemDataService<HouseDto> problemDataService, ILogger<HousesService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(HouseDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи House с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи House с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<HouseDto> entities, int batchSize = 1000)
        {
            var batch = new List<HouseDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<HouseDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<HouseDto> entities, int batchSize = 100)
        {
            var batch = new List<HouseDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<HouseDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(HouseDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(HouseDto record)
        {
