...
{
    public class SteadsParamsService: ISteadsParamsService
    {
        private readonly ILogger<SteadsParamsService> _logger;
        private readonly ISteadsParamsRepo _repository;
        private readonly IProblemDataService<ParamDto> _problemDataService;

        public SteadsParamsService(ISteadsParamsRepo repository, IProblemDataService<ParamDto> problemDataService, ILogger<SteadsParamsService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(ParamDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи SteadsParams с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи SteadsParams  с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
