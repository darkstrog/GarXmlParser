...
{
    public class NormativeDocsService: INormativeDocsService
    {
        private readonly ILogger<NormativeDocsService> _logger;
        private readonly INormativeDocsRepo _repository;
        private readonly IProblemDataService<NormativeDocDto> _problemDataService;

        public NormativeDocsService(INormativeDocsRepo repository, IProblemDataService<NormativeDocDto> problemDataService, ILogger<NormativeDocsService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(NormativeDocDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи NormativeDoc с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи NormativeDoc  с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<NormativeDocDto> entities, int batchSize = 1000)
        {
            var batch = new List<NormativeDocDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<NormativeDocDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<NormativeDocDto> entities, int batchSize = 100)
        {
            var batch = new List<NormativeDocDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<NormativeDocDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(NormativeDocDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(NormativeDocDto record)
        {
