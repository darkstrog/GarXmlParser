...
{
    public class NormativeDocKindsService: INormativeDocKindsService
    {
        private readonly ILogger<NormativeDocKindsService> _logger;
        private readonly INormativeDocKindRepo _repository;
        private readonly IProblemDataService<NormativeDocKindDto> _problemDataService;

        public NormativeDocKindsService(INormativeDocKindRepo repository, IProblemDataService<NormativeDocKindDto> problemDataService, ILogger<NormativeDocKindsService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(NormativeDocKindDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи NormativeDocKind с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи NormativeDocKind с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<NormativeDocKindDto> entities, int batchSize = 1000)
        {
            var batch = new List<NormativeDocKindDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<NormativeDocKindDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<NormativeDocKindDto> entities, int batchSize = 100)
        {
            var batch = new List<NormativeDocKindDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<NormativeDocKindDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(NormativeDocKindDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(NormativeDocKindDto record)
        {
