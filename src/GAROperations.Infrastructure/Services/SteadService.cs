...
{
    public class SteadsService: ISteadsService
    {
        private readonly ILogger<SteadsService> _logger;
        private readonly ISteadRepo _repository;
        private readonly IProblemDataService<SteadDto> _problemDataService;

        public SteadsService(ISteadRepo repository, IProblemDataService<SteadDto> problemDataService, ILogger<SteadsService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(SteadDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи Stead с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи Stead с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<SteadDto> entities, int batchSize = 1000)
        {
            var batch = new List<SteadDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<SteadDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<SteadDto> entities, int batchSize = 100)
        {
            var batch = new List<SteadDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<SteadDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(SteadDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(SteadDto record)
        {
