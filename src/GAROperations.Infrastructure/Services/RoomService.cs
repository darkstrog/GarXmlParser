...
namespace GAROperations.Infrastructure.Services
{
    public class RoomService : IRoomService
    {
        ...
        private readonly IProblemDataService<RoomDto> _problemDataService;

        public RoomService(IRoomRepo repository, IProblemDataService<RoomDto> problemDataService, ILogger<RoomService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(RoomDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи Room с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи Room с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<RoomDto> entities, int batchSize = 1000)
        {
            var batch = new List<RoomDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<RoomDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<RoomDto> entities, int batchSize = 100)
        {
            var batch = new List<RoomDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<RoomDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(RoomDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(RoomDto record)
        {
