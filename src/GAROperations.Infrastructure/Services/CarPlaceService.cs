...
{
    public class CarPlaceService: ICarPlaceService
    {
        private readonly ILogger<CarPlaceService> _logger;
        private readonly ICarPlaceRepo _repository;
        private readonly IProblemDataService<CarPlaceDto> _problemDataService;

        public CarPlaceService(ICarPlaceRepo repository, IProblemDataService<CarPlaceDto> problemDataService, ILogger<CarPlaceService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(CarPlaceDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи CarPlace с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи CarPlace  с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<CarPlaceDto> entities, int batchSize = 1000)
        {
            var batch = new List<CarPlaceDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<CarPlaceDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<CarPlaceDto> entities, int batchSize = 100)
        {
            var batch = new List<CarPlaceDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<CarPlaceDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(CarPlaceDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(CarPlaceDto record)
        {
