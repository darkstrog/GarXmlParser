...
{
    public class AddressObjectParamsService : IAddressObjectParamsService
    {
        private readonly ILogger<AddressObjectParamsService> _logger;
        private readonly AddressObjectParamsRepo _repository;
        private readonly IProblemDataService<ParamDto> _problemDataService;

        public AddressObjectParamsService(AddressObjectParamsRepo repository, IProblemDataService<ParamDto> problemDataService, ILogger<AddressObjectParamsService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(ParamDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи AddrObjParams с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи AddrObjParams  с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<ParamDto> entities, int batchSize = 1000)
        {
            var batch = new List<ParamDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<ParamDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<ParamDto> entities, int batchSize = 100)
        {
            var batch = new List<ParamDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<ParamDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(ParamDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(ParamDto record)
        {
