...
{
    public class AddressObjectTypeService: IAddressObjectTypeService
    {
        private readonly ILogger<AddressObjectTypeService> _logger;
        private readonly IAddressObjectTypeRepo _repository;
        private readonly IProblemDataService<AddressObjectTypeDto> _problemDataService;
        public AddressObjectTypeService(IAddressObjectTypeRepo repo,
                                                IProblemDataService<AddressObjectTypeDto> problemDataService,
                                                ILogger<AddressObjectTypeService> logger)
        {
            _logger = logger;
            _repository = repo;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(AddressObjectTypeDto entity)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<AddressObjectTypeDto> entities, int batchSize = 1000)
        {
            var batch = new List<AddressObjectTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<AddressObjectTypeDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <param name="batchSize"></param>
        /// <returns></returns>
        public async Task InsertAddressObjectsBulk(IEnumerable<AddressObjectTypeDto> entities, int batchSize = 100)
        {
            var batch = new List<AddressObjectTypeDto>(batchSize);
        ...
        /// <param name="batch"></param>
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<AddressObjectTypeDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(AddressObjectTypeDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(AddressObjectTypeDto record)
        {
