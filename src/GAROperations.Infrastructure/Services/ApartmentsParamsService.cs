...
{
    public class ApartmentsParamsService: IApartmentsParamsService
    {
        private readonly ILogger<ApartmentsParamsService> _logger;
        private readonly IApartmentsParamsRepo _repository;
        private readonly IProblemDataService<ParamDto> _problemDataService;

        public ApartmentsParamsService(IApartmentsParamsRepo repository, IProblemDataService<ParamDto> problemDataService, ILogger<ApartmentsParamsService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(ParamDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи ApartmentsParams с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи ApartmentsParams с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
