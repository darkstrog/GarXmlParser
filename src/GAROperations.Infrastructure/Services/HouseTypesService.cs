...
namespace GAROperations.Infrastructure.Services
{
    ...
    {
        private readonly ILogger<HouseTypesService> _logger;
        private readonly IHouseTypesRepo _repository;
        private readonly IProblemDataService<HouseTypeDto> _problemDataService;

        public HouseTypesService(IHouseTypesRepo repository, IProblemDataService<HouseTypeDto> problemDataService, ILogger<HouseTypesService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(HouseTypeDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи HouseType с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи HouseType с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<HouseTypeDto> entities, int batchSize = 1000)
        {
            var batch = new List<HouseTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<HouseTypeDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<HouseTypeDto> entities, int batchSize = 100)
        {
            var batch = new List<HouseTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<HouseTypeDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(HouseTypeDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(HouseTypeDto record)
        {
