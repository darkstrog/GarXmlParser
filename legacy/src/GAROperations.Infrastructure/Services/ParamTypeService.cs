...
namespace GAROperations.Infrastructure.Services
{
    ...
    {
        private readonly ILogger<ParamTypeService> _logger;
        private readonly IParamTypeRepo _repository;
        private readonly IProblemDataService<ParamTypeDto> _problemDataService;

        public ParamTypeService(IParamTypeRepo repository, IProblemDataService<ParamTypeDto> problemDataService, ILogger<ParamTypeService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(ParamTypeDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи ParamType с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи ParamType с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<ParamTypeDto> entities, int batchSize = 1000)
        {
            var batch = new List<ParamTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<ParamTypeDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<ParamTypeDto> entities, int batchSize = 100)
        {
            var batch = new List<ParamTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<ParamTypeDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(ParamTypeDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(ParamTypeDto record)
        {
