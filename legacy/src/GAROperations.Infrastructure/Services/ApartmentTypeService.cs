...
namespace GAROperations.Infrastructure.Services
{
    ...
    {
        private readonly ILogger<ApartmentTypeService> _logger;
        private readonly IApartmentTypeRepo _repository;
        private readonly IProblemDataService<ApartmentTypeDto> _problemDataService;

        public ApartmentTypeService(IApartmentTypeRepo repository, IProblemDataService<ApartmentTypeDto> problemDataService, ILogger<ApartmentTypeService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(ApartmentTypeDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи ApartmentType с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи ApartmentType  с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<ApartmentTypeDto> entities, int batchSize = 1000)
        {
            var batch = new List<ApartmentTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<ApartmentTypeDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<ApartmentTypeDto> entities, int batchSize = 100)
        {
            var batch = new List<ApartmentTypeDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<ApartmentTypeDto> batch)
        {
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(ApartmentTypeDto problematicRecord)
        {
            try
            {
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(ApartmentTypeDto record)
        {
