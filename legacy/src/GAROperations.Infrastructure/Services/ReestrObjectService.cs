...
{
    public class ReestrObjectService : IReestrObjectService
    {
        private readonly ILogger<ReestrObjectService> _logger;
        private readonly IReestrObjectRepo _repository;
        private readonly IProblemDataService<ReestrObjectDto> _problemDataService;

        public ReestrObjectService(IReestrObjectRepo repository, IProblemDataService<ReestrObjectDto> problemDataService, ILogger<ReestrObjectService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(ReestrObjectDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи ReestrObject с OBJECTID: {ID}, CHANGEID: {CHANGEID} в базу", data.OBJECTID, data.CHANGEID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи ReestrObject с OBJECTID: {ID}, CHANGEID: {CHANGEID}", data.OBJECTID, data.CHANGEID);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<ReestrObjectDto> entities, int batchSize = 1000)
        {
            var batch = new List<ReestrObjectDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<ReestrObjectDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<ReestrObjectDto> entities, int batchSize = 100)
        {
            var batch = new List<ReestrObjectDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<ReestrObjectDto> batch)
        {
            ...
            catch (Exception ex)
            {
                ...
                foreach (var record in batch)
                {
                    ...
                    {
                        _logger.LogError(singleEx, "Не удалось записать в базу {ID} ,отправляем в сервис обработки проблемных данных", record.OBJECTID);
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(ReestrObjectDto problematicRecord)
        {
            try
            {
                await _problemDataService.ProcessProblematicRecord(problematicRecord, null);
                _logger.LogInformation("Проблемная запись {ID} отправлена на анализ", problematicRecord.OBJECTID);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Не удалось отправить проблемную запись {ID} в сервис обработки", problematicRecord.OBJECTID);
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(ReestrObjectDto record)
        {
            var csvLine = $"{DateTime.Now},{record.OBJECTID},{record.CHANGEID}";
