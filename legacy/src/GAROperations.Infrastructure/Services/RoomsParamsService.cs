...
{
    public class RoomsParamsService: IRoomParamsService
    {
        private readonly ILogger<RoomsParamsService> _logger;
        private readonly IRoomParamsRepo _repository;
        private readonly IProblemDataService<ParamDto> _problemDataService;

        public RoomsParamsService(IRoomParamsRepo repository, IProblemDataService<ParamDto> problemDataService, ILogger<RoomsParamsService> logger)
        {
            _logger = logger;
        ...
        public async Task InsertDataAsync(ParamDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи RoomsParams с ID: {ID} в базу", data.ID);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи RoomsParams с ID: {ID}", data.ID);
                throw;
            }
        }
        /// <summary>
