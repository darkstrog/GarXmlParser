...
{
    public class ObjectLevelService: IObjectLevelService
    {
        private readonly ILogger<ObjectLevelService> _logger;
        private readonly IObjectLevelRepo _repository;
        private readonly IProblemDataService<ObjectLevelDto> _problemDataService;

        public ObjectLevelService(IObjectLevelRepo repository, IProblemDataService<ObjectLevelDto> problemDataService, ILogger<ObjectLevelService> logger)
        {
            _logger = logger;
            _repository = repository;
            _problemDataService = problemDataService;
        }
        public async Task InsertDataAsync(ObjectLevelDto data)
        {
            try
            {
                _logger?.LogInformation("Добавление одной записи ObjectLevel с ID: {level} в базу", data.LEVEL);
                await _repository.InsertAsync(data);
            }
            catch (Exception)
            {
                _logger?.LogError("Ошибка При добавлении записи ObjectLevel с ID: {level}", data.LEVEL);
                throw;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public async Task InsertDataBulkAsync(IAsyncEnumerable<ObjectLevelDto> entities, int batchSize = 1000)
        {
            var batch = new List<ObjectLevelDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessBatchWithFallback(List<ObjectLevelDto> batch, int originalBatchSize)
        {
            try
            {
        ...
        /// <returns></returns>
        public async Task InsertEntitiesBulk(IEnumerable<ObjectLevelDto> entities, int batchSize = 100)
        {
            var batch = new List<ObjectLevelDto>(batchSize);
        ...
        /// <returns></returns>
        private async Task ProcessSingleBatchWithIsolation(List<ObjectLevelDto> batch)
        {
            ...
            catch (Exception ex)
            {
                ...
                foreach (var record in batch)
                {
                    ...
                    {
                        _logger.LogError(singleEx, "Не удалось записать в базу {level} ,отправляем в сервис обработки проблемных данных", record.LEVEL);
        ...
        /// <returns></returns>
        private async Task HandleProblematicRecord(ObjectLevelDto problematicRecord)
        {
            try
            {
                await _problemDataService.ProcessProblematicRecord(problematicRecord, null);
                _logger.LogInformation("Проблемная запись {level} отправлена на анализ", problematicRecord.LEVEL);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Не удалось отправить проблемную запись {level} в сервис обработки", problematicRecord.LEVEL);
        ...
        /// <returns></returns>
        private async Task WriteToCsvFallback(ObjectLevelDto record)
        {
            var csvLine = $"{DateTime.Now},{record.LEVEL}";
