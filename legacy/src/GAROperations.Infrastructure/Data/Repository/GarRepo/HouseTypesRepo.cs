...
{
    public class HouseTypeRepo: IHouseTypesRepo
    {
        private readonly string _connectionStringGar;
        private readonly ILogger _logger;

        #region sql-запросы
        private const string IsExistQuery = "SELECT EXISTS(SELECT 1 FROM house_types WHERE id = @Id)";
        private const string GetByIdAsyncQuery = "SELECT * FROM house_types WHERE id = @Id";
        private const string CreateAsyncQuery = @"INSERT INTO house_types (id, name, shortname, description, 
                                                                           updatedate, startdate, enddate, isactive)
                                                  VALUES(@id, @name, @shortname, @description, 
                                                                           @updatedate, @startdate, @enddate, @isactive)";

        private const string CreateBulkAsyncQuery = @"INSERT INTO house_types (id, name, shortname, description, 
                                                                           updatedate, startdate, enddate, isactive)
                                                  VALUES(@id, @name, @shortname, @description, 
                                                                           @updatedate, @startdate, @enddate, @isactive)";
        #endregion
        public HouseTypeRepo(string connectionStringGar, ILogger logger)
        {
            _connectionStringGar = connectionStringGar;
            _logger = logger;
        }

        public async Task InsertAsync(HouseTypeDto entity)
        {
            using var db = new NpgsqlConnection(_connectionStringGar);
            await db.ExecuteAsync(CreateAsyncQuery, entity);
        }

        public async Task<bool> IsExist(HouseTypeDto entity)
        {
            using (var db = new NpgsqlConnection(_connectionStringGar))
            {
                try
        ...
        public async Task InsertBulkAsync(IEnumerable<HouseTypeDto> entities)
        {
        ...
        public Task UpdateAsync(HouseTypeDto entity)
        {
        ...
        public Task<IEnumerable<HouseTypeDto>> GetPagedAsync(int page, int pageSize, Expression<Func<HouseTypeDto, bool>>? filter = null)
        {
            throw new NotImplementedException();
        }

        public Task<long> GetCountAsync(Expression<Func<HouseTypeDto, bool>>? filter = null)
        {
            throw new NotImplementedException();
        }
    }
