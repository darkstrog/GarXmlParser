...
{
    public class AddressObjectParamsRepo : IAddressObjectParamsRepo
    {
        private readonly string _connectionStringGar;
        private readonly ILogger _logger;

        #region sql-запросы
        private const string IsExistQuery = "SELECT EXISTS(SELECT 1 FROM addr_obj_params WHERE id = @Id)";
        private const string GetByIdAsyncQuery = @"SELECT id, objectid, changeid, changeidend, typeid,
                                                          value, updatedate, startdate, enddate
                                                   FROM addr_obj_params
                                                   WHERE id = @Id";
        private const string CreateAsyncQuery = @"INSERT INTO addr_obj_params (id, objectid, changeid, changeidend, typeid,
                                                                               value, updatedate, startdate, enddate)
                                                  VALUES(@id, @objectid, @changeid, @changeidend, @typeid,
                                                                               @value, @updatedate, @startdate, @enddate)";

        private const string CreateBulkAsyncQuery = @"INSERT INTO addr_obj_params (id, objectid, changeid, changeidend, typeid,
                                                                               value, updatedate, startdate, enddate)
                                                  VALUES(@id, @objectid, @changeid, @changeidend, @typeid,
                                                                               @value, @updatedate, @startdate, @enddate)";
        #endregion
        public AddressObjectParamsRepo(string connectionStringGar, ILogger<AddressObjectParamsRepo> logger)
        {
