...
{
    public class SteadsParamsRepo: ISteadsParamsRepo
    {
        private readonly string _connectionStringGar;
        private readonly ILogger _logger;

        #region sql-запросы
        private const string IsExistQuery = "SELECT EXISTS(SELECT 1 FROM steads_params WHERE id = @Id)";
        private const string GetByIdAsyncQuery = @"SELECT id, objectid, changeid, changeidend, typeid,
                                                          value, updatedate, startdate, enddate
                                                   FROM steads_params
                                                   WHERE id = @Id";
        private const string CreateAsyncQuery = @"INSERT INTO steads_params (id, objectid, changeid, changeidend, 
                                                                             typeid, value, updatedate, startdate, enddate)
                                                  VALUES(@id, @objectid, @changeid, @changeidend, 
                                                                             @typeid, @value, @updatedate, @startdate, @enddate)";

        private const string CreateBulkAsyncQuery = @"INSERT INTO steads_params (id, objectid, changeid, changeidend, 
                                                                             typeid, value, updatedate, startdate, enddate)
                                                  VALUES(@id, @objectid, @changeid, @changeidend, 
                                                                             @typeid, @value, @updatedate, @startdate, @enddate)";
        #endregion
        public SteadsParamsRepo(string connectionStringGar, ILogger logger)
        {
        ...
        public async Task<bool> IsExist(ParamDto entity)
        {
            using (var db = new NpgsqlConnection(_connectionStringGar))
            {
                try
                {
                    ...
                    if (exists)
                    {
                        _logger.LogInformation("SteadsParam с Id: {ID}: уже существует в базе", entity.ID);
                        return true;
                    }

                    _logger.LogInformation("SteadsParam с Id: {ID}: не найден", entity.ID);
                    return false;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Не удалась проверить существование SteadsParam с Id: {ID}", entity.ID);
                    throw;
                }
            }
        }
